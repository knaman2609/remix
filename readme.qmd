...
description: "Upload any song → AI splits it into stems → swap any stem's style → mix and export."
format: gfm
---
...
# Set your Gemini API key (DO NOT commit actual keys to version control)
...
### GPU note
Demucs runs on CPU but is slow (~2-3 min per song). With a CUDA GPU, separation takes 15-30 seconds. The Docker setup auto-detects NVIDIA GPUs when using the NVIDIA Container Toolkit with docker-compose GPU reservations (requires nvidia-docker runtime).
